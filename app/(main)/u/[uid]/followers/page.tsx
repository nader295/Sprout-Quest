...
import { apiGetFollowersList, apiGetUser } from "@/lib/api/client";
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [followers, profile] = await Promise.all([
        apiGetFollowersList(uid, 200),
        apiGetUser(uid),
      ]);
      setUsers(Array.isArray(followers) ? followers : []);
    ...
    <div className="mx-auto w-full max-w-2xl px-3 py-3 sm:px-4 sm:py-4 lg:px-6">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-base font-bold text-foreground">{t('followers.title')}</h1>
          {profileName && (
            <p className="text-xs text-muted-foreground">{t('followers.desc2')} {profileName}</p>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm font-medium text-foreground">
            {search ? t('followers.noResults') : t('followers.noFollowersYet')}
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            {!search && t('followers.emptyMsg')}
          </p>
        </div>
      ) : (
